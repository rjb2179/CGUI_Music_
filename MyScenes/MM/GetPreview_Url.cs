...
public class GetPreview_Url : MmBaseResponder { 

    private List<string> trackPreview_Url = new List<string>();
    string accessToken = "";
    string tokenType = "";
    int count = 0;
    ...
    IEnumerator GetSpotifyTrackID(string albumID)
    {
        ...
        {
            trackPreview_Url.Add(jTracks["items"][i]["preview_url"].str);
        }

        foreach (string Artist in trackPreview_Url)
        {
            Debug.Log(Artist);
        }
    }
