...
public class GetTrackArtists : MmBaseResponder
{

    private List<string> trackArtists = new List<string>();
    string accessToken = "";
    string tokenType = "";
    int count = 0;
    ...
    IEnumerator GetSpotifyTrackID(string albumID)
    {
        ...
        {
            trackArtists.Add(jTracks["items"][i]["artists"][0]["name"].str);
            trackArtists.Add(jTracks["items"][i]["artists"][0]["id"].str);
        }

        foreach (string str in trackArtists)
        {
            Debug.Log(str);
        }
    }
