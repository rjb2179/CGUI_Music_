...
public class SpotifyGenerator : MonoBehaviour {
    ...
    private float hoveringOffset = .01f;
    private float radius = 1f;
    private float angularRadianSpacing = .25f; 
    ...
    private void GenerateObjects(List<string> ids, List<string> names, List<Texture2D> textures, string objectType)
    {
        ...
        for (int i = 0; i < numberOfObjects; i++)
        {
            ...
            go.GetComponent<TouchableCircleObject_Oculus>().spotifyData = myObject;
    ...
    private void Spawn()
    {
        for (int i = 0; i < numberOfObjects; i++)
        {
            ...
            // Vector3 outVector = -Mathf.Sin(angle) * this.transform.right + Mathf.Cos(angle) * this.transform.up;
            // Vector3 targetPosition = outVector * radius + transform.position - hoveringOffset * transform.forward;
            // Quaternion targetRotation = Quaternion.LookRotation(transform.forward, outVector);
            ...
            Vector3 targetPosition = outVector * radius + transform.position - hoveringOffset * transform.forward;
            Quaternion targetRotation = STONES.transform.rotation * Quaternion.Inverse(myObjects[i].GO.transform.rotation);
